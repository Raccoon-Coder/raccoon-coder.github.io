...
class SnakeGame {
  ...
  moveByOne() {
    ...
    if (
      this.snake.eatSelf() ||
      this.snake.isOutBounds(this.gridWidth - 1, this.gridHeight - 1)
    ) {
      this.endGame();
      return;
    }
    if (
      this.snake.x === this.foodLocation.x &&
      this.snake.y === this.foodLocation.y
    ) {
      this.score += 1;
      document.getElementById("js-current-score").innerHTML = this.score;
      this.addFood();
      ...
      this.interval = setInterval(
        this.moveByOne,
        Math.max(1000 * 0.9 ** this.score, 50)
      );
    }
    this.paint();
  }
